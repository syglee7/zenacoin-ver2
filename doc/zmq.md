...
The Zenacoin Core daemon can be configured to act as a trusted "border
router", implementing the zenacoin wire protocol and relay, making
...
The ZeroMQ feature in Zenacoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building zenacoind:
...
For instance:
    ...
    $ zenacoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/zenacoind.tx.raw \
...
These options can also be provided in zenacoin.conf.
...
From the perspective of zenacoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into zenacoind directly. Furthermore, no information is
...
using. Zenacoind appends an up-counting sequence number to each
