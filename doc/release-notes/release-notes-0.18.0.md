Zenacoin Core version 0.18.0 is now available from:

  <https://zenacoincore.org/bin/zenacoin-core-0.18.0/>
...
`/Applications/Zenacoin-Qt` (on Mac) or `zenacoind`/`zenacoin-qt` (on
Linux).
...
recommended to use Zenacoin Core on unsupported systems.

Zenacoin Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Zenacoin Core does not yet change appearance when
...
`contrib/init/zenacoind.service`, it has been changed to use
`/var/lib/zenacoind` as the data directory instead of
`~zenacoin/.zenacoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/zenacoind` will
exist has enough space (check using `df -h /var/lib/zenacoind`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/zenacoin/zenacoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://zenacoincore.org/en/contact/) the Zenacoin Core
...
The systemd init file (`contrib/init/zenacoind.service`) has been changed
to use `/var/lib/zenacoind` as the data directory instead of
`~zenacoin/.zenacoin`. This change makes Zenacoin Core more consistent with
...
`datadir` option in `/etc/zenacoin/zenacoin.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/zenacoin/zenacoin.conf`.


Documentation
-------------

- A new short [document](https://github.com/zenacoin/zenacoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/zenacoin/zenacoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/zenacoin/zenacoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/zenacoin/zenacoin/blob/master/doc/zenacoin-conf.md)
  about the `zenacoin.conf` file describes how to use it to configure
  Zenacoin Core.

- A new document introduces Zenacoin Core's BIP174 [Partially-Signed
  Zenacoin Transactions
  (PSBT)](https://github.com/zenacoin/zenacoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/zenacoin/zenacoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Zenacoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Zenacoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/zenacoin/zenacoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/zenacoin/zenacoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Zenacoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Zenacoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/zenacoin/zenacoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/zenacoin/zenacoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart zenacoind with the `-deprecatedrpc=generate`
...
- A new `zenacoin-wallet` tool is now distributed alongside Zenacoin
...
This section describes planned changes to Zenacoin Core that may affect
other Zenacoin software and services.

- Since version 0.16.0, Zenacoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Zenacoin Core 0.20 (expected about
  a year after 0.18), Zenacoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Zenacoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Zenacoin Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Zenacoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Zenacoin Core
...
- A sub-project of Zenacoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Zenacoin Core.  See their [project
  page](https://github.com/zenacoin-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Zenacoin Core's own implementation, although entropy
  gathered by Zenacoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Zenacoin Core a
...
- On macOS, Zenacoin Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `zenacoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libzenacoin_server.a` (jnewbery)
...
- #13926 [Tools] zenacoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like zenacoind (MarcoFalke)
...
- #14521 qt, docs: Fix `zenacoin-qt -version` output formatting (hebasto)
...
- #11625 Add ZenacoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libzenacoin_server` (laanwj)
...
- #15154 Configure: zenacoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/zenacoin-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Zenacoin Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libzenacoin_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in zenacoin-util-test.py (isghe)
...
- #12775 Integration of property based testing into Zenacoin Core (Christewart)
...
- #12246 Bugfix: Only run zenacoin-tx tests when zenacoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_zenacoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_zenacoin` (MarcoFalke)
...
- #14186 zenacoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_zenacoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update zenacoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/zenacoin-conf.md (hebasto)
...
- #15354 Add missing `zenacoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct zenacoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support zenacoin folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/zenacoin/).
