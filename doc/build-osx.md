...
To build Zenacoin Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode).
...
1. Clone the Zenacoin Core source code:
    ```shell
...
2.  Build Zenacoin Core:
    ...
    Configure and build the headless Zenacoin Core binaries as well as the GUI (if Qt is found).
...
## Disable-wallet mode
When the intention is to run only a P2P node without a wallet, Zenacoin Core may be
...
```shell
mkdir -p "/Users/${USER}/Library/Application Support/Zenacoin"

touch "/Users/${USER}/Library/Application Support/Zenacoin/zenacoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Zenacoin/zenacoin.conf"
```

The first time you run zenacoind, it will start downloading the blockchain. This process could
