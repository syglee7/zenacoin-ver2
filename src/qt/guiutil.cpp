...
#include <qt/zenacoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Zenacoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new ZenacoinAddressEntryValidator(parent));
    widget->setCheckValidator(new ZenacoinAddressCheckValidator(parent));
}

bool parseZenacoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no zenacoin: URI
    if(!uri.isValid() || uri.scheme() != QString("zenacoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!ZenacoinUnits::parse(ZenacoinUnits::ZNC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseZenacoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseZenacoinURI(uriInstance, out);
}

QString formatZenacoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("zenacoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(ZenacoinUnits::format(ZenacoinUnits::ZNC, info.amount, false, ZenacoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openZenacoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", ZENACOIN_CONF_FILENAME));
    ...
    /* Open zenacoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Zenacoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Zenacoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Zenacoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Zenacoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "zenacoin.desktop";
    return GetAutostartDir() / strprintf("zenacoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a zenacoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Zenacoin\n";
        else
            optionFile << strprintf("Name=Zenacoin (%s)\n", chain);
