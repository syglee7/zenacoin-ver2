...
const char * const ZENACOIN_CONF_FILENAME = "zenacoin.conf";
const char * const ZENACOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/zenacoin/zenacoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //zenacoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", ZENACOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Zenacoin
    // macOS: ~/Library/Application Support/Zenacoin
    // Unix-like: ~/.zenacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Zenacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Zenacoin";
#else
    // Unix-like
    return pathRet / ".zenacoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", ZENACOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Zenacoin Core copyright is not removed by accident
    if (copyright_devs.find("Zenacoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Zenacoin Core developers";
    }
